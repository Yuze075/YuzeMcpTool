...
" + userCode + @"
            ...
            CancellationToken cancellationToken)
        {
            ...
            var completedTask = await Task.WhenAny(completion.Task, delayTask, cancellationTask);
            if (completedTask != completion.Task)
            {
                if (completedTask == cancellationTask)
                    return Error("evalJsCode was canceled.");
...
", "UnityEval.bootstrap");
        ...
        private void Tick()
        {
            lock (_syncRoot)
            {
                ...
                {
                    Debug.LogError($"[UnityEvalTool] PuerTS tick error in session {_sessionId}: {ex.Message}");
                }
            }
        }
        ...
        private void DisposeEnv(string reason)
        {
            ...
            {
                Debug.LogWarning($"[UnityEvalTool] Failed to dispose eval VM ({reason}): {ex.Message}");
            }
            finally
            {
                _scriptEnv = null;
