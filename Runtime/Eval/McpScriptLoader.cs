...
namespace YuzeToolkit
{
    internal sealed class McpScriptLoader : ILoader, IModuleChecker
    {
        ...
        public bool FileExists(string filepath)
        {
            ...
            {
                if (McpToolCatalog.TryGetCSharpModuleSource(toolName, out _)) return true;
                return McpToolCatalog.IsResourceToolEnabled(toolName) && TryLoadResourceText(modulePath, out _, out _);
            }
        ...
        public string ReadFile(string filepath, out string debugpath)
        {
            ...
            if (IsToolIndexPath(modulePath))
            {
                ...
                return McpToolCatalog.GenerateIndexModuleSource();
            }

            if (TryGetToolModuleName(modulePath, out var toolName) &&
                McpToolCatalog.TryGetCSharpModuleSource(toolName, out var virtualSource))
            {
            ...
            if (TryGetToolModuleName(modulePath, out var resourceToolName) &&
                !McpToolCatalog.IsResourceToolEnabled(resourceToolName))
            {
                debugpath = filepath;
                return string.Empty;
            }
...
#if UNITY_EDITOR
            if (McpToolCatalog.TryReadEditorResourceText(path, out text, out debugpath))
                return true;
#endif
