...
namespace YuzeToolkit
{
    public sealed class McpServerBehaviour : MonoBehaviour
    {
        ...
        [SerializeField, Tooltip("Whether MCP requests must include the configured token in Authorization: Bearer or X-UnityEvalTool-Token.")]
        private bool requireToken;

        [SerializeField, Tooltip("Token required by MCP requests when token authentication is enabled. Leave empty to generate one on start.")]
