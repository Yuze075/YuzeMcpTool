    ...
    internal sealed class TcpMcpTransport : IMcpTransport
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (errors.Count > 0)
                    Debug.LogWarning($"[UnityEvalTool] Some MCP TCP bind addresses failed: {string.Join("; ", errors)}");
        ...
        private async Task<ParsedHttpRequest> ReadHttpRequestAsync(NetworkStream stream)
        {
            ...
            while (headerEnd < 0)
            {
                var read = await ReadWithTimeoutAsync(stream, temp, 0, temp.Length);
                if (read <= 0)
            ...
            {
                var read = await ReadWithTimeoutAsync(stream, bodyBytes, offset, contentLength - offset);
                if (read <= 0)
        ...
        private async Task<int> ReadWithTimeoutAsync(NetworkStream stream, byte[] buffer, int offset, int count)
        {
            var readTask = stream.ReadAsync(buffer, offset, count);
            var timeoutTask = Task.Delay(ReadTimeout, _cancellationToken);
            var completed = await Task.WhenAny(readTask, timeoutTask);
            if (completed != readTask)
                throw new IOException($"TCP read timed out after {ReadTimeout.TotalSeconds:0}s.");
        ...
        private static McpTransportResponse JsonError(int statusCode, string message) =>
            McpTransportResponse.Json(statusCode, EvalData.Obj(("error", message)), EmptyHeaders());
