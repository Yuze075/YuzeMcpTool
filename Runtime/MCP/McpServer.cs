...
namespace YuzeToolkit
{
    public sealed class McpServer : IDisposable
    {
        ...
        private const string ServerName = "UnityEvalTool";
        private const string ManualOwnerId = "manual";
        private const string TokenHeader = "X-UnityEvalTool-Token";
        ...
        private EvalSessionRegistry _sessions = new();
        private McpServerOptions _options = new();
        private McpServerState _state = new();
        private IMcpTransport? _transport;
        private CancellationTokenSource? _cancellation;
        private EvalExecutor _evalTool;

        private McpServer()
        {
            _evalTool = new EvalExecutor(ToEvalOptions(_options));
        }
        ...
        public void StartWithOwner(string ownerId, McpServerOptions? options = null)
        {
            lock (_syncRoot)
            {
                ...
                _evalTool = new EvalExecutor(ToEvalOptions(_options));
                _sessions.Dispose();
                _sessions = new EvalSessionRegistry();
                ...
                try
                {
                    ...
                    if (!result.Success)
                    {
                        ...
                        Debug.LogWarning($"[UnityEvalTool] MCP transport '{_transport.Name}' did not start: {result.Error}");
                        _transport.Stop();
                        _transport = null;
                    ...
                    Debug.Log($"[UnityEvalTool] MCP server started via {_transport.Name}: {_state.Endpoint}");
                }
                catch (Exception ex)
                {
                    ...
                    _cancellation = null;
                    _startOwners.Remove(ownerId);
                    Debug.LogError($"[UnityEvalTool] Failed to start MCP server: {ex.Message}");
                }
            }
        }
        ...
        private static EvalOptions ToEvalOptions(McpServerOptions options) =>
            new()
            {
                MaxRequestBodyBytes = options.MaxRequestBodyBytes,
                DefaultEvalTimeoutSeconds = options.DefaultEvalTimeoutSeconds
            };
        ...
        private async Task<McpTransportResponse> ProcessRequestAsync(McpTransportRequest request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (!IsAuthorized(request))
                    return McpTransportResponse.Json(401, JsonRpcError(null, -32004, "Unauthorized MCP request."), headers);
                ...
                {
                    return McpTransportResponse.Json(404, EvalData.Obj(("error", "Not Found")), headers);
                }
                ...
                {
                    headers["Allow"] = "POST, OPTIONS, DELETE";
                    return McpTransportResponse.Json(405, EvalData.Obj(("error", "Method Not Allowed")), headers);
                }
        ...
        private async Task<McpHttpResult> HandleSingleJsonRpcAsync(
            ...
            CancellationToken cancellationToken)
        {
            var message = EvalData.AsObject(parsed);
            if (message == null)
            ...
            var method = EvalData.GetString(message, "method") ?? string.Empty;
            ...
            {
                "ping" => new McpHttpResult(200, JsonRpcResult(id, EvalData.Obj())),
                "tools/list" => new McpHttpResult(200, JsonRpcResult(id, EvalData.Obj(("tools", EvalData.Arr(EvalExecutor.ToolDefinition()))))),
        ...
        {
            message.TryGetValue("id", out var id);
            var parameters = EvalData.AsObject(message.TryGetValue("params", out var p) ? p : null);
            var requestedProtocol = parameters != null ? EvalData.GetString(parameters, "protocolVersion") : null;
            var clientInfo = parameters != null ? EvalData.AsObject(parameters.TryGetValue("clientInfo", out var c) ? c : null) : null;
            var clientName = clientInfo != null ? EvalData.GetString(clientInfo, "name") ?? string.Empty : string.Empty;
            ...
            return JsonRpcResult(id, EvalData.Obj(
                ("protocolVersion", protocol),
                ("capabilities", EvalData.Obj(
                    ("tools", EvalData.Obj())
                )),
                ("serverInfo", EvalData.Obj(
                    ("name", ServerName),
                    ("version", "1.0.0")
                ))
            ));
        ...
        private async Task<object> HandleToolCallAsync(object? id, Dictionary<string, object?> message, EvalSession session, CancellationToken cancellationToken)
        {
            var parameters = EvalData.AsObject(message.TryGetValue("params", out var p) ? p : null);
            if (parameters == null)
                return JsonRpcError(id, -32602, "Invalid tools/call params.");

            var toolName = EvalData.GetString(parameters, "name") ?? string.Empty;
            if (toolName != "evalJsCode")
                return JsonRpcError(id, -32602, $"Unknown tool '{toolName}'. This server only exposes evalJsCode.");

            var args = EvalData.AsObject(parameters.TryGetValue("arguments", out var a) ? a : null) ?? new Dictionary<string, object?>();
        ...
        private bool TryResolveSession(McpTransportRequest request, out EvalSession session, out (int HttpStatusCode, int Code, string Message) error)
        {
            var sessionId = request.GetHeader("mcp-session-id") ?? string.Empty;
            if (string.IsNullOrEmpty(sessionId))
            {
        ...
        private static bool TryValidateProtocolHeader(McpTransportRequest request, EvalSession session, out (int Code, string Message) error)
        {
        ...
        private object BuildHealthObject()
        {
            ...
            return EvalData.Obj(
                ...
                ("auth", EvalData.Obj(("requireToken", _options.RequireToken))),
        ...
        private static object JsonRpcResult(object? id, object? result) =>
            EvalData.Obj(("jsonrpc", "2.0"), ("result", result), ("id", id));

        private static object JsonRpcError(object? id, int code, string message) =>
            EvalData.Obj(
                ("jsonrpc", "2.0"),
                ("error", EvalData.Obj(("code", code), ("message", message))),
                ("id", id)
            );
        ...
        private static Dictionary<string, string> BuildCorsHeaders()
        {
            return new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
            {
                ...
                ["Access-Control-Allow-Headers"] = "Authorization, Content-Type, MCP-Session-Id, MCP-Protocol-Version, X-UnityEvalTool-Token, mcp-session-id, mcp-protocol-version",
                ["Access-Control-Expose-Headers"] = "MCP-Session-Id, mcp-session-id"
            };
        }

        private bool IsAuthorized(McpTransportRequest request)
        {
            if (!_options.RequireToken) return true;
            if (string.IsNullOrWhiteSpace(_options.Token)) return false;

            var token = request.GetHeader(TokenHeader);
            if (string.IsNullOrWhiteSpace(token))
            {
                var authorization = request.GetHeader("Authorization") ?? string.Empty;
