...
* 初始版本更新和提交
* 新增基于 `[EvalTool]` C# class 和 `Resources/tools` 的动态工具目录，项目或其他包可通过 C# 注册或新增 `.mjs` helper 扩展工具。
...
* C# tool 不再要求继承 `IMcpTool`；运行时注册改为 `[EvalTool(name, description)]` + `EvalToolRegistry.Register<TTool>() where TTool : class, new()`。
...
* `evalJsCode` 结果转换移动到 MCP 服务端出口：基础类型、List、Dictionary 统一作为 JSON text content 返回，UnityObject 和 C# 自定义对象由 `EvalValueFormatter` 摘要。
