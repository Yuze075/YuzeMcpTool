    ...
    public sealed class SerializedTool
    {
        ...
        public Dictionary<string, object?> get(object target, string propertyPath = "")
        {
            ...
            if (string.IsNullOrWhiteSpace(propertyPath))
            {
                ...
                return EvalData.Obj(("target", SummarizeObject(obj)), ("properties", props));
            }
        ...
        private static object? GetPropertyValue(SerializedProperty property)
        {
            return property.propertyType switch
            {
                ...
                SerializedPropertyType.Rect => EvalData.Obj(("x", property.rectValue.x), ("y", property.rectValue.y), ("width", property.rectValue.width), ("height", property.rectValue.height)),
                SerializedPropertyType.Bounds => EvalData.Obj(("center", ToolUtilities.Vector3ToObject(property.boundsValue.center)), ("size", ToolUtilities.Vector3ToObject(property.boundsValue.size))),
                _ => property.hasVisibleChildren ? "(children)" : property.stringValue
            };
        }

        [EvalFunction("Set one serialized property.")]
                ...
                case SerializedPropertyType.Rect:
                    if (EvalData.AsObject(value) is { } rect)
                        property.rectValue = new Rect(
                            EvalData.GetFloat(rect, "x", property.rectValue.x),
                            EvalData.GetFloat(rect, "y", property.rectValue.y),
                            EvalData.GetFloat(rect, "width", property.rectValue.width),
                            EvalData.GetFloat(rect, "height", property.rectValue.height));
                    break;
        ...
        [EvalFunction("Set multiple serialized properties.")]
        public Dictionary<string, object?> setMany(object target, object changes, bool confirm = false)
        {
            if (!confirm) throw new InvalidOperationException("Serialized property writes require confirm: true.");
            var obj = ResolveUnityObject(target);
            if (obj == null) throw new InvalidOperationException("Target UnityEngine.Object was not found.");
            var changeList = (EvalData.AsArray(changes) ?? new List<object?>())
        ...
        [EvalFunction("Resize array property.")]
        ...
        [EvalFunction("Insert array element.")]
        ...
        [EvalFunction("Delete array element.")]
