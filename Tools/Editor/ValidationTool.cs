...
namespace YuzeToolkit
{
    ...
    [McpTool("validation", "Project health checks for missing scripts, loaded-scene references, and conventions.")]
    public sealed class ValidationTool
    {
        [McpFunction("Run all validations.")]
        ...
        [McpFunction("Find missing scripts.")]
        ...
        private static Dictionary<string, object?> GetMissingScriptsResult()
        {
            ...
            foreach (var go in Resources.FindObjectsOfTypeAll<GameObject>().Where(go => go != null && ToolUtilities.IsUsableSceneObject(go, true)))
            {
                ...
                if (count <= 0) continue;
                issues.Add(McpData.Obj(("scope", "scene"), ("count", count), ("gameObject", ToolUtilities.SummarizeGameObject(go, false))));
            }
            ...
            foreach (var guid in AssetDatabase.FindAssets("t:Prefab"))
            {
                ...
                if (prefab == null)
                {
                    issues.Add(McpData.Obj(("scope", "prefab"), ("path", path), ("error", "Prefab asset could not be loaded.")));
                    continue;
                }
                var count = GameObjectUtility.GetMonoBehavioursWithMissingScriptCount(prefab);
                if (count > 0)
                    issues.Add(McpData.Obj(("scope", "prefab"), ("path", path), ("count", count)));
            }

            return McpData.Obj(("count", issues.Count), ("issues", issues));
        }

        [McpFunction("Find broken serialized object references in loaded scenes.")]
        ...
        [McpFunction("Check SerializeField Tooltip convention.")]
