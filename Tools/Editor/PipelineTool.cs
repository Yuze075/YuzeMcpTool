...
namespace YuzeToolkit
{
    ...
    [McpTool("pipeline", "Package Manager, Test Runner, and BuildPipeline workflows.")]
    public sealed class PipelineTool
    {
        [McpFunction("List packages.")]
        public Dictionary<string, object?> listPackages()
        {
            var packages = PackageInfo.GetAllRegisteredPackages()
                .Select(package => (object?)PipelineRequestStore.SummarizePackage(package))
                .ToList();
            return McpData.Obj(("count", packages.Count), ("packages", packages));
        }

        [McpFunction("Add package.")]
        ...
        [McpFunction("Read test run status.")]
        ...
        [McpFunction("Read build settings.")]
        public Dictionary<string, object?> getBuildSettings()
        {
            var scenes = EditorBuildSettings.scenes
                .Select(scene => (object?)McpData.Obj(("path", scene.path), ("enabled", scene.enabled), ("guid", scene.guid.ToString())))
                .ToList();
            return McpData.Obj(
        ...
        private static class PipelineRequestStore
        {
            ...
            public static Dictionary<string, object?> GetPackageRequest(string id)
            {
                lock (SyncRoot)
                {
                    if (!PackageRequests.TryGetValue(id, out var tracked))
                        return McpData.Obj(("found", false), ("id", id));
            ...
            public static string TrackBuild(Dictionary<string, object?> summary)
            {
                var id = Guid.NewGuid().ToString("N");
                lock (SyncRoot)
                    Builds[id] = McpData.Obj(("found", true), ("id", id), ("finishedAtUtc", DateTime.UtcNow.ToString("O")), ("summary", summary));
                return id;
            }
            ...
            {
                lock (SyncRoot)
                    return Builds.TryGetValue(id, out var build) ? build : McpData.Obj(("found", false), ("id", id));
            }
