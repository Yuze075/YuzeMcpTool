...
namespace YuzeToolkit
{
    ...
    [McpTool("diagnostics", "Read-only cameras, physics, graphics, UI, and loaded texture diagnostics.")]
    public sealed class DiagnosticsTool
    {
        [McpFunction("List cameras.")]
        public Dictionary<string, object?> listCameras()
        {
            var cameras = Resources.FindObjectsOfTypeAll<Camera>()
                ...
                .ToList();
            return McpData.Obj(("count", cameras.Count), ("cameras", cameras));
        }
        ...
        [McpFunction("Read 2D/3D physics settings.")]
        public Dictionary<string, object?> getPhysicsState()
        {
            var colliders2D = Resources.FindObjectsOfTypeAll<Collider2D>()
                ...
                .Select(collider => (object?)McpData.Obj(
        ...
        [McpFunction("Read render pipeline and quality state.")]
        ...
        [McpFunction("List UI canvases and EventSystems.")]
        public Dictionary<string, object?> listCanvases()
        {
            var canvases = Resources.FindObjectsOfTypeAll<Canvas>()
                ...
                .Select(canvas => (object?)McpData.Obj(
                    ("name", canvas.name),
                ...
                .ToList();
            return McpData.Obj(("canvasCount", canvases.Count), ("canvases", canvases));
        }

        [McpFunction("List loaded textures.")]
        public Dictionary<string, object?> listLoadedTextures(int limit = 100)
        {
            ...
            var textures = Resources.FindObjectsOfTypeAll<Texture>()
                ...
                .Take(limit)
                .Select(texture => (object?)McpData.Obj(
                    ("name", texture.name),
                ...
                .ToList();
            return McpData.Obj(("count", textures.Count), ("textures", textures));
        }
    }
}
